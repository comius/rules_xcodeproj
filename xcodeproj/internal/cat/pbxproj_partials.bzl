...
def _write_schemes(
        *,
        ...
        generator_name,
        install_path):
    """Collects `.xcscheme`s from various actions into a single directory.

    Args:
        actions: `ctx.actions`.
        automatic_xcschemes: A `File`s for the automatically generated
            `.xcscheme`s directory.
...
cp -c -R "$1/" "$output"
""",
        inputs = [automatic_xcschemes],
        outputs = [output],
        progress_message = "Collecting schemes for {}".format(install_path),
        mnemonic = "CollectXCSchemes",
