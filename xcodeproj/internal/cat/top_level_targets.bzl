...
def _calculate_mergeable_info(
        *,
        ...
        product_type,
        target_inputs):
    # Target merging isn't supported in BwX mode
    if build_mode == "xcode":
        return None
...
def _cc_mergeable_info(*, id, mergeable_info):
    return struct(
        ...
        hdrs = mergeable_info.inputs.hdrs,
...
def process_top_level_target(
    ...
    if test_host_target_info and product_type == _UNIT_TEST_PRODUCT_TYPE:
...
def _process_focused_top_level_target(
    ...
    (
        ...
        params_files,
    ) = pbxproj_partials.write_compiler_build_settings(
        actions = ctx.actions,
    ...
    extension_target_infos = [
        ...
        # FIXME: Is this the correct way to handle unfocused extensions?
    ...
    app_clips = [
        ...
        # FIXME: Is this the right way to handle unfocused app clips?
        if extension_target[XcodeProjInfo].xcode_target
    ]
